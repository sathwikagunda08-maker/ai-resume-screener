...
def pred():
    if 'resume' in request.files:
        ...
        try:
            match_score, match_feedback, category, recommended_job = analyze_resume_match(resume_text, job_description)
...
def analyze_resume_match(resume_text, job_description):
    ...
    Provide a match percentage, categorization of the resume, a specific recommended job title, and detailed feedback about the fit.
    ...
    Category: [broad professional category, e.g., Software Development, Data Science, Marketing]
    Recommended Job: [specific job title suitable for the candidate]
    ...
    try:
        ...
        # Extract recommended job
        recommended_job_match = re.search(r'\*?\*?Recommended Job:?[\s\*]*([^\n\r]*)', content, re.IGNORECASE)
        recommended_job = recommended_job_match.group(1).strip() if recommended_job_match else "N/A"
    ...
    print(match_score, category, recommended_job, feedback)
    return match_score, feedback, category, recommended_job
...
def bulk_analyze():
    ...
    success_count = len([r for r in results if r['match_score'] != 'Error'])
    message = f"Successfully analyzed {success_count} resumes." if success_count > 0 else "No resumes were successfully analyzed."
    
    return render_template('bulk_resume.html', results=results, message=message)
